        ...
        public void Configure(IApplicationBuilder app, IHostingEnvironment env, ILoggerFactory loggerFactory)
        {
            ...
            var proxyOptions = new ProxyOptions
            {
                ProxyRules = new List<ProxyRule> {
                    new ProxyRule {
                        Matcher = (uri, context) => uri.AbsoluteUri.Contains("/api1"),
                        Modifier = (msg ,user) => {
                    ...
                    new ProxyRule {
                        Matcher = (uri, context) => uri.AbsoluteUri.Contains("/api2"),
                        Modifier = (msg ,user) => {
                            var uri = new UriBuilder(msg.RequestUri) {
                                Port = 5002,
                                Path = "/api/values"
                    ...
                    new ProxyRule {
                        Matcher = (uri, context) => uri.AbsoluteUri.Contains("/authenticate"),
                        Modifier = (msg ,user) => {
                            var uri = new UriBuilder(msg.RequestUri) {
                                Port = 5000
                            };
